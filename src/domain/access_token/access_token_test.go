...
func TestGetNewAccessToekn(t *testing.T) {
	...
	assert.False(t, at.IsExpired(), "brand new accss token should not be expired")
	assert.EqualValues(t, "", at.AccessToken, "new access token should not have defined access token id")
	assert.True(t, at.UserID == 0, "new accees token should not have an associated user id")
